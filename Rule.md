...
（2）建造范围：玩家每个已有建筑周围 8 格内以及主基地边缘4格宽度内（即玩家0横纵坐标<11，玩家1横纵坐标>188，且不在基地范围内）为建造范围，新建筑只能建于建造范围内。
...
（3）建筑升级并不会改变建筑种类。例如，在科技2级时，升级一个0级bool，它会变成一个1级bool，还可以继续升级成2级bool，但不能超过3级。但是如果在科技2级时建造bool将会直接建造2级bool，花费更多的资源，拥有更强的属性。
...
（1）建造力是一个只与科技水平相关的值，初始为80，每提升一级科技增加60。
...
# 所有建筑与单位的详细设定
	...
	1.所有给出基础值的实际数值为基础值*（1+时代阶段数*0.5），即bit级别的数值为基础值，此后每级增加50%; 
	  例如，BIT科技时建造的0级布尔属性刚好为下表所示基础属性，而AI科技建造的建筑，一经建造就是4级，属性为下表属性*3
	2.关于AOE：AOE为2表示可以攻击某一格和其相邻格内所有单位，即与该格距离小于2，而非小于等于2，以此类推
	3.数据类单位只会被数据攻击打到，实体单位同理

|资源建筑|基础生命值|基础资源采集值|基础资源|
|-------|---------|------------|-------|
|码农|100|10|100|

|生产建筑|基础生命值|单位|生产范围|生产CD|解锁时代|基础资源|基础建造力|
|-------|---------|--|--------|-------|-------|---------|------|
|香农|150|比特流|10|3|比特|150|15|
|戴维南|200|电压源|5|6|电路|160|16|
|诺顿|180|电流源|5|6|电路|160|16|
|冯诺依曼|200|ENIAC|15|15|处理器|200|20|
|伯纳斯李|150|数据包|20|3|网络|250|25|
|高锟|160|光纤|15|8|网络|300|30|
|图灵|300|图灵机|15|20|AI|600|60|
|托尼斯塔克|250|奥创|10|20|AI|600|60|

|单位|类型|行动模式|基础生命值|基础攻击力|攻击范围|移动速度|
|----|---|-------|---------|---------|-------|------|
|比特流|数据|推塔|20|10|16|16|
|电压源|实体|推塔|60|16|24|12|
|电流源|实体|冲锋|100|160|6|20|
|ENIAC|实体|抗线|500|15|10|6|
|数据包|数据|冲锋|70|300|6|24|
|光纤|实体|推塔|40|15|40|16|
|图灵机|数据|抗线|800|10|12|4|
|奥创|实体|推塔|100|500|20|12|

|防御建筑|基础生命值|基础攻击力|攻击CD|射程|攻击目标|AOE|解锁时代|基础资源|基础建造力|特效|
|-------|---------|---------|---|------|----|-------|-------|---------|---|----|
|布尔|300|16|1|36|数据|0|比特|150|15|布尔代数：有一半的几率攻击无效|
|欧姆|280|20|3|30|实体|6|电路|200|20|欧姆定律：同时击中电压源和电流源时对所有击中单位造成4倍伤害|
|摩尔|225|4|1|60|实体|0|处理器|225|22|摩尔定律：优先攻击上回合目标且对一个目标连续攻击时每次伤害*2|
|蒙特卡罗|300|25|2|40|实体|0|算法|200|20|蒙特卡罗方法：对单位造成的伤害会乘以一个0~2之间的随机数|
|拉里罗伯茨|180|6|1|50|全部|8|网络|250|25|阿帕网：优先攻击数据包且对数据包造成的伤害*3|
|罗伯特卡恩|450|5|1|36|数据|0|网络|450|45|TCP\IP协议：攻击任何目标均附加其最大生命值百分之10（*科技因数）的伤害|
|马斯克|1000|0|1|24|全部|0|AI|500|50|AI末日论：使其攻击范围内的AI兵种无法移动与攻击|
|霍金|400|∞|2|20|全部|2|AI|500|50|黑洞：你也看到这伤害了，消灭1格和邻格内所有单位|
